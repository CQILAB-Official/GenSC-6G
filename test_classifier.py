...
root_folder = "dataset/AIDER" if args.dataset == 'aider' else "dataset/EuroSAT_RGB"
...
print(f"Model complexity: {sum(p.numel() for p in model.parameters() if p.requires_grad)}")

# # PSNR Test for SNR 1-30
# snr_values = [5, 10, 15, 20, 30]
# for i in snr_values:
#     test(model, dataloader_test, test_snr=i, is_psnr=True)
